...
module.exports.getTweets = (req, res) => {
  ...
  // console.log(
  //   `SELECT u.id as userId, u.avatar as avatar, u.email as email, u.name as name, u.username as username, t.id as tweetId, t.tweet as tweet, t.likeCount as likeCount, t.__createdtime__ as tweetTime, l.isLiked as isLiked FROM tweets.tweets as t INNER JOIN tweets.user as u ON u.id = t.userId LEFT OUTER JOIN tweets.tweetLikes as l on t.id = l.tweetId AND l.userId = "${userId}" ORDER BY t.__createdtime__ DESC`
  // );
  db.query(
...
module.exports.getTrendingTweets = (req, res) => {
  db.query(
    `SELECT u.id as userId, u.avatar as avatar, u.name as name, u.username as username, t.id as tweetId, t.tweet as tweet, t.__createdtime__ as tweetTime FROM tweets.tweets as t INNER JOIN tweets.user as u ON u.id = t.userId ORDER BY t.likeCount DESC LIMIT 5`
  )
    .then((result) => {
      const tweets = result.data;
      // console.log(tweets);
...
module.exports.likeTweet = (req, res) => {
  if (req.isAuthenticated()) {
    ...
    db.query(
      `SELECT * FROM tweets.tweetLikes WHERE tweetId = "${tweetId}" and userId = "${userId}"`,
      (err, response) => {
        if (err) {
          console.log(err);
          ...
          if (response.data.length == 0) {
            ...
            // console.log(
            //   `UPDATE tweets.tweetLikes SET isLiked = ${isLiked} WHERE tweetId = "${tweetId}" and userId = "${userId}"`
            // );
            db.query(
              `UPDATE tweets.tweetLikes SET isLiked = ${isLiked} WHERE tweetId = "${tweetId}" and userId = "${userId}"`,
              (err, response) => {
...
function changeLikeCount(tweetId, isLiked, res) {
  // console.log(
  //   `UPDATE tweets.tweets SET likeCount = likeCount ${
  //     isLiked ? "+" : "-"
  //   } 1 WHERE id = "${tweetId}"`
  // );
  db.query(
